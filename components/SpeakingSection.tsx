...
export const SpeakingSection: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Grid adjustment: 1 col on mobile, 2 cols on tablet/desktop. 
          Inside the Right Panel (which is approx 66% width on desktop), 2 cols fits well. */}
      <div className="grid gap-6 sm:grid-cols-2">
        {data.items.map((item, idx) => (
          <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col h-full">
